...
#define CM_UART_TX_GPIO 84U
#define CM_UART_TX_GPIO_CFG GPIO_84_UARTA_TX
...
static void cpu1_init_cm_uart_tx_pin(void)
{
    GPIO_setPinConfig(CM_UART_TX_GPIO_CFG);
    GPIO_setPadConfig(CM_UART_TX_GPIO, GPIO_PIN_TYPE_STD);
}
...
static void cpu1_boot_cpu2(void)
{
#ifdef _FLASH
...
static void cpu1_run_cpu2_handshake(void)
{
    uint32_t poll_count;
...
void main(void)
{
    ...
    cpu1_init_heartbeat();
    cpu1_init_cm_uart_tx_pin();
    cpu1_run_cpu2_handshake();
